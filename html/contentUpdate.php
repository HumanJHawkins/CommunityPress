<?php
  ...
  if ($action == 'delete') {
    ...
    $sql = 'SELECT contentInsert("' . $contentTitle . '","' . $contentDescription . '","' . $contentText . '","' . $contentURL . '",' . $userID . ')';
    $row = getOnePDORow($pdo, $sql);
    $_SESSION['lastURL'] = 'contentView.php?action=edit&pageContentID=' . $row[0];
    header('Location: ' . $_SESSION['lastURL']);
    exit();
  } else if ($action == 'update') {
    $sql = 'SELECT contentUpdate(' . $_POST["pageContentID"] . ',"' . $contentTitle . '","' . $contentDescription . '","' . $contentText . '","' . $contentURL . '",' . $userID . ')';
    $result = getOnePDORow($pdo, $sql);
